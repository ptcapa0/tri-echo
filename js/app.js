...
import {calibrateShot,computeGestureMetrics,crossedPowerThreshold,mapPullToPower} from './physics-calibration.js';
...
class Game{
 ...
 shoot(vx,vy,powerRatio){
  ...
  this.lastShotSpeed=Math.hypot(vx,vy);this.lastNormalizedPower=powerRatio;
...
function dragPower(){if(!drag)return{speed:0,normalizedPower:0,effectivePull:0};return mapPullToPower(Math.hypot(drag.screenStart.x-drag.screen.x,drag.screenStart.y-drag.screen.y),{...game.shotProfile,...game.gestureProfile})}
function down(e){if(!game?.canAcceptGameplayInput())return;audio.unlock();const p=map(e),b=game.table.balls[0];if(Math.hypot(p.x-b.x,p.y-b.y)<100){drag={start:{x:b.x,y:b.y},p,screenStart:{x:e.clientX,y:e.clientY},screen:{x:e.clientX,y:e.clientY},id:e.pointerId,normalizedPower:0};try{canvas.setPointerCapture(e.pointerId)}catch{}if(!data.tutorial)coach(1)}}
function move(e){if(!drag||e.pointerId!==drag.id)return;drag.p=map(e);drag.screen={x:e.clientX,y:e.clientY};const power=dragPower(),threshold=crossedPowerThreshold(drag.normalizedPower,power.normalizedPower);if(threshold!==null){buzz(threshold===1?10:4);if(threshold===1)audio.tone(520,.035,'sine',.025)}drag.normalizedPower=power.normalizedPower}
function up(e){if(!drag||e.pointerId!==drag.id)return;drag.p=map(e);drag.screen={x:e.clientX,y:e.clientY};const dx=drag.start.x-drag.p.x,dy=drag.start.y-drag.p.y,d=Math.hypot(dx,dy),power=dragPower();drag=null;if(!power.speed||!d||!game.canAcceptGameplayInput()||!game.shoot(dx/d*power.speed,dy/d*power.speed,power.normalizedPower))return;audio.hit(power.normalizedPower);if(!data.tutorial){coach(3);data.tutorial=true;save(data);setTimeout(()=>$('#tutorial').classList.add('hidden'),2200)}}
