...
function down(e){if(!game?.canAcceptGameplayInput())return;audio.unlock();const p=map(e),b=game.table.balls[0];if(Math.hypot(p.x-b.x,p.y-b.y)<100){drag={start:{x:b.x,y:b.y},p,id:e.pointerId,maxed:false};canvas.setPointerCapture(e.pointerId);if(!data.tutorial)coach(1)}}
