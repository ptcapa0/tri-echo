...
function down(e){if(!game?.canAcceptGameplayInput())return;e.preventDefault();audio.unlock();drag={originScreen:{x:e.clientX,y:e.clientY},currentScreen:{x:e.clientX,y:e.clientY},id:e.pointerId,normalizedPower:0};try{canvas.setPointerCapture(e.pointerId)}catch{}if(!data.tutorial)coach(1)}
function move(e){if(!drag||e.pointerId!==drag.id)return;drag.currentScreen={x:e.clientX,y:e.clientY};const power=floatingPull(),threshold=crossedPowerThreshold(drag.normalizedPower,power.normalizedPower);if(threshold!==null){buzz(threshold===1?10:4);if(threshold===1)audio.tone(520,.035,'sine',.025)}drag.normalizedPower=power.normalizedPower}
function up(e){if(!drag||e.pointerId!==drag.id)return;drag.currentScreen={x:e.clientX,y:e.clientY};const pull=floatingPull(),{shotDirection}=pull;drag=null;if(!pull.speed||!game.canAcceptGameplayInput()||!game.shoot(shotDirection.x*pull.speed,shotDirection.y*pull.speed,pull.normalizedPower))return;audio.hit(pull.normalizedPower);if(!data.tutorial){coach(3);data.tutorial=true;save(data);setTimeout(()=>$('#tutorial').classList.add('hidden'),2200)}}
