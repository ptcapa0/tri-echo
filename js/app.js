...
import {Physics,STEP,echoFromPath} from './physics.js';
import {calibrateShot,computeGestureMetrics,crossedPowerThreshold,estimateTableStoppingDistance} from './physics-calibration.js';
import {deriveFloatingPull} from './floating-pull.js';
import {AIM_PROFILES,deriveAimPreview} from './aim-preview.js';
...
class Game{
 constructor(){this.mode=data.mode in MODES?data.mode:'golf';this.info=MODES[this.mode];const daily=this.mode==='daily'?dailyChallengeConfig():null;this.dailyDayKey=daily?.dayKey||null;this.difficulty=daily?.difficulty||data.difficulty;this.tableStyle=daily?.tableStyle||(this.info.tableChoice?(data.tableStyle||'echo'):'snooker');this.trainingDiscipline=data.trainingDiscipline||'golf';this.trickDiscipline=data.trickDiscipline||'golf';this.seedBase=daily?.seed??Date.now()>>>0;this.holeIndex=0;this.strokes=0;this.totalStrokes=0;this.totalPar=0;this.score=0;this.streak=0;this.results=[];this.inventory=freshPowerInventory();this.activePower=null;this.ruleState={phase:'open',reds:15,colourIndex:0};this.finished=false;this.roundTasks=new RoundTaskController();this.interactionGate=new InteractionGate();this.newHole()}
 ...
 newHole(){
  ...
  this.table=generateTable(seed,this.difficulty,this.adaptive(),dims.w,dims.h,{tableStyle,ballSet,traditional:this.isTraditional(),targetType,pocketModel:this.info.kind==='trick'?'physical':undefined});this.table.rails=echoes;
...
function checkAchievements(){if(game.results.some(r=>r.strokes<r.par))addAchievement('Birdie','Termina abaixo do par');if(game.streak>=3)addAchievement('Linha Quente','Três buracos ao par ou melhor');if(game.physics.cushions>=CUSHION_REQUIREMENTS.geometer)addAchievement('Geómetra','Emboca após três ressaltos')}
...
function gestureForCanvas(){const r=canvas.getBoundingClientRect();return computeGestureMetrics(r.width||innerWidth,r.height||innerHeight)}
function resize(){const r=$('#stage').getBoundingClientRect(),dpr=Math.min(2,devicePixelRatio||1);canvas.width=Math.round(r.width*dpr);canvas.height=Math.round(r.height*dpr);canvas.style.width=`${r.width}px`;canvas.style.height=`${r.height}px`;if(game)game.gestureProfile=gestureForCanvas();placeContact()}
function floatingPull(){
 if(!drag||!game)return{distanceCss:0,normalizedPull:0,normalizedPower:0,speed:0,shotDirection:{x:0,y:0}};
 const r=canvas.getBoundingClientRect(),t=game.table;
 return deriveFloatingPull({originScreen:drag.originScreen,currentScreen:drag.currentScreen,gestureProfile:{...game.shotProfile,...game.gestureProfile},worldScale:{x:t.w/r.width,y:t.h/r.height}});
}
function down(e){if(contactPointer!==null||movePointer!==null||!gameplayPointer.acquire(e.pointerId,game?.canAcceptGameplayInput()===true))return;e.preventDefault();audio.unlock();drag={originScreen:{x:e.clientX,y:e.clientY},currentScreen:{x:e.clientX,y:e.clientY},normalizedPower:0};try{canvas.setPointerCapture(e.pointerId)}catch{}if(!data.tutorial)coach(1)}
function move(e){if(!gameplayPointer.owns(e.pointerId)||!drag)return;drag.currentScreen={x:e.clientX,y:e.clientY};const power=floatingPull(),threshold=crossedPowerThreshold(drag.normalizedPower,power.normalizedPower);if(threshold!==null){buzz(threshold===1?10:4);if(threshold===1)audio.tone(520,.035,'sine',.025)}drag.normalizedPower=power.normalizedPower}
function up(e){if(!gameplayPointer.owns(e.pointerId)||!drag)return;drag.currentScreen={x:e.clientX,y:e.clientY};const pull=floatingPull(),{shotDirection}=pull;releaseGameplayPointer(e.pointerId);if(!pull.speed||!game.canAcceptGameplayInput()||!game.shoot(shotDirection.x*pull.speed,shotDirection.y*pull.speed,pull.normalizedPower))return;audio.hit(pull.normalizedPower);if(!data.tutorial){coach(3);data.tutorial=true;save(data);setTimeout(()=>$('#tutorial').classList.add('hidden'),2200)}}
...
function contactDown(e){if(!game?.canAcceptGameplayInput()||gameplayPointer.pointerId!==null||contactPointer!==null||movePointer!==null)return;e.preventDefault();e.stopPropagation();contactPointer=e.pointerId;$('#cueFace').setPointerCapture(e.pointerId);setContact(e);buzz(4)}
...
function moveControlDown(e){if(!game?.canAcceptGameplayInput()||gameplayPointer.pointerId!==null||contactPointer!==null||movePointer!==null)return;e.preventDefault();e.stopPropagation();movePointer=e.pointerId;$('#moveContact').setPointerCapture(e.pointerId)}
...
function releaseCapture(element,id){if(id!=null)try{if(element.hasPointerCapture(id))element.releasePointerCapture(id)}catch{}}
function releaseGameplayPointer(pointerId){if(!gameplayPointer.release(pointerId))return false;drag=null;releaseCapture(canvas,pointerId);return true}
function cancelGameplayPointer(e){const pointerId=e?e.pointerId:gameplayPointer.clear();if(pointerId==null||(e&&!gameplayPointer.release(pointerId)))return false;drag=null;releaseCapture(canvas,pointerId);return true}
function cancelContactPointer(e){if(contactPointer==null||(e&&e.pointerId!==contactPointer))return false;const pointerId=contactPointer;contactPointer=null;releaseCapture($('#cueFace'),pointerId);return true}
...
function render(){if(!game)return;const W=canvas.clientWidth,H=canvas.clientHeight,t=game.table,dpr=Math.min(2,devicePixelRatio||1),sx=W/t.w,sy=H/t.h;ctx.setTransform(dpr*sx,0,0,dpr*sy,0,0);ctx.fillStyle=t.tableStyle==='snooker'?'#09261f':'#071a21';ctx.fillRect(0,0,t.w,t.h);ctx.strokeStyle=t.tableStyle==='snooker'?'#b98752':'#31535a';ctx.lineWidth=t.tableStyle==='snooker'?12:4;if(t.pocketModel!=='physical')ctx.strokeRect(t.bounds.l,t.bounds.t,t.bounds.r-t.bounds.l,t.bounds.b-t.bounds.t);ctx.globalAlpha=.08;ctx.strokeStyle='#5cf3dc';for(let y=120;y<t.h;y+=120){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(t.w,y);ctx.stroke()}ctx.globalAlpha=1;if(t.hole)drawHole(t.hole,game.hybridPhase!=='carom');if(t.pocketModel==='physical')drawPhysicalPockets(t.pocketGeometry);else for(const p of t.pockets||[])drawPocket(p);if(t.frictionZone){ctx.fillStyle=t.frictionZone.factor>1?'#ffca6815':'#5cf3dc12';ctx.strokeStyle=t.frictionZone.factor>1?'#ffca6860':'#5cf3dc60';roundRect(t.frictionZone.x,t.frictionZone.y,t.frictionZone.w,t.frictionZone.h,18,true,true)}for(const r of t.rails){ctx.strokeStyle='#5cf3dc88';ctx.shadowColor='#5cf3dc';ctx.shadowBlur=13;ctx.lineWidth=9;ctx.beginPath();ctx.moveTo(r.a.x,r.a.y);ctx.lineTo(r.b.x,r.b.y);ctx.stroke();ctx.shadowBlur=0}for(const o of t.obstacles){ctx.fillStyle='#18343d';ctx.strokeStyle='#ffca68';ctx.lineWidth=4;ctx.beginPath();ctx.arc(o.x,o.y,o.r,0,Math.PI*2);ctx.fill();ctx.stroke()}if(drag)drawAim(t);for(const b of t.balls)drawBall(b);if(drag)drawFloatingPull();for(const p of particles){ctx.globalAlpha=Math.max(0,p.life);ctx.fillStyle=p.c;ctx.fillRect(p.x,p.y,5,5)}ctx.globalAlpha=1}
function drawPhysicalPockets(geometry){
 ctx.save();ctx.fillStyle='#010304';
 for(const p of geometry.pockets){ctx.beginPath();p.outline.forEach((v,i)=>i?ctx.lineTo(v.x,v.y):ctx.moveTo(v.x,v.y));ctx.closePath();ctx.fill()}
 ctx.strokeStyle='#b98752';ctx.lineCap='round';
 for(const s of geometry.segments){ctx.lineWidth=s.radius*2;ctx.beginPath();ctx.moveTo(s.a.x,s.a.y);ctx.lineTo(s.b.x,s.b.y);ctx.stroke()}
 ctx.restore();
}
...
function aimPreview(){const pull=floatingPull(),cue=game?.table.balls[0];if(!game||!cue)return null;const profile=AIM_PROFILES[game.difficulty]||AIM_PROFILES.normal;return deriveAimPreview({table:game.table,cueBall:cue,shotDirection:pull.shotDirection,shotSpeed:pull.speed,assistance:profile.assistance,maxPhysicalTravel:estimateTableStoppingDistance({table:game.table,cueBall:cue,shotDirection:pull.shotDirection,initialSpeed:pull.speed}),trace:game.activePower==='trace'})}
function drawAim(t){const b=t.balls[0],pull=floatingPull(),pow=pull.normalizedPower,aim=aimPreview();if(aim?.segments.length){const s=aim.segments[0];ctx.setLineDash([11,9]);ctx.strokeStyle=aim.firstHit?'#efffffe0':'#efffffa8';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(s.from.x,s.from.y);ctx.lineTo(s.to.x,s.to.y);ctx.stroke();ctx.setLineDash([]);if(aim.firstHit){ctx.fillStyle=aim.firstHit.type==='BALL'?'#ffca68':'#ff6d9e';ctx.beginPath();ctx.arc(s.to.x,s.to.y,6,0,Math.PI*2);ctx.fill()}if(aim.ghostCue&&AIM_PROFILES[game.difficulty]?.ghost){ctx.save();ctx.globalAlpha=.32;ctx.strokeStyle='#efffff';ctx.lineWidth=3;ctx.beginPath();ctx.arc(aim.ghostCue.x,aim.ghostCue.y,aim.ghostCue.r,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=.8;ctx.fillStyle='#ffca68';ctx.beginPath();ctx.arc(aim.ghostCue.contact.x,aim.ghostCue.contact.y,4,0,Math.PI*2);ctx.fill();ctx.restore()}}ctx.strokeStyle=pow>.82?'#ffca68':'#5cf3dc';ctx.lineWidth=7;ctx.beginPath();ctx.arc(b.x,b.y,b.r+14,-Math.PI/2,-Math.PI/2+Math.PI*2*pow);ctx.stroke();if(Math.hypot(contact.x,contact.y)>.05){ctx.fillStyle='#ff6d9e';ctx.beginPath();ctx.arc(b.x+contact.x*b.r*.58,b.y+contact.y*b.r*.58,3.5,0,Math.PI*2);ctx.fill()}ctx.fillStyle='#efffff';ctx.font='800 22px system-ui';ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(`${Math.round(pow*100)}%`,b.x,b.y-b.r-20)}
function drawFloatingPull(){const r=canvas.getBoundingClientRect(),dpr=Math.min(2,devicePixelRatio||1),origin={x:drag.originScreen.x-r.left,y:drag.originScreen.y-r.top},current={x:drag.currentScreen.x-r.left,y:drag.currentScreen.y-r.top},power=floatingPull().normalizedPower;ctx.save();ctx.setTransform(dpr,0,0,dpr,0,0);ctx.globalAlpha=.88;ctx.strokeStyle=power>.82?'#ffca68':'#5cf3dc';ctx.fillStyle='#071a21dd';ctx.lineWidth=2.5;ctx.setLineDash([7,6]);ctx.beginPath();ctx.moveTo(origin.x,origin.y);ctx.lineTo(current.x,current.y);ctx.stroke();ctx.setLineDash([]);ctx.shadowColor='#5cf3dc';ctx.shadowBlur=9;ctx.beginPath();ctx.arc(origin.x,origin.y,10,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.shadowBlur=0;ctx.globalAlpha=.72;ctx.beginPath();ctx.arc(current.x,current.y,5,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.restore()}
