...
- Controlos: Floating Pull pode começar em qualquer ponto jogável do canvas, incluindo sobre a branca; o gesto é inverso à tacada e a distância em píxeis CSS determina a força, sem depender da posição da branca. O primeiro pointer válido é o único owner da tacada até terminar ou ser cancelado; pointers gameplay secundários são ignorados. O controlo circular móvel define seguimento, recuo e efeito lateral.
...
- Integridade de colisões: cada timestep externo mantém 1/180 s e usa até 16 microsteps adaptativos determinísticos, derivados da velocidade relativa e do menor diâmetro ativo. Episódios físicos deduplicam eventos de regras, casos degenerados usam normais estáveis e o stress fixo cobre bolas, tabelas, bumpers e Echo Rails. A geometria tradicional das bolsas permanece fora deste contrato.
- Potência: um solver sobre a simulação real encontra a menor velocidade que cobre o `SHOT_ENVELOPE` (`maxCushions: 3`, `reserveDiameters: 3`) e margem geométrica; a curva contínua começa em zero e a física fundamental não muda com a dificuldade. Esta garantia aplica-se à superfície física base; zonas de atrito e outros modificadores deliberados podem alterar o alcance efetivo e devem ser considerados pela geração futura.
